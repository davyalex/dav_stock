...
class RapportController extends Controller
{
    ...
    public function venteParProduit(Request $request)
    {
        ...
        $ventesProduit = $query->orderBy('created_at', 'desc')->get();

        $produits = Produit::active()->get();
        $caisses = Caisse::all();

        // Calcul du total vendu pour ce produit
        $totalVendu = $ventesProduit->sum(function($item) {
            return $item->quantite * $item->prix_unitaire;
        });

        if ($request->classement == 'plus_vendu') {
            $ventesProduit = $ventesProduit->sortByDesc('quantite');
        } elseif ($request->classement == 'moins_vendu') {
            $ventesProduit = $ventesProduit->sortBy('quantite');
        }

        return view('backend.pages.rapport.produit', compact('ventesProduit', 'produits', 'caisses', 'totalVendu', 'produitId'));
    }
